...
Given: $children = \beta_0 + \beta_1educ+\beta_2age+\beta_3age^2 +u$, 

```{r echo=FALSE, results='asis'}
EducIV <- lm(educ ~ frsthalf + age + agesq, data = fertil)
frsthalf.IV <- EducIV$fitted.values
FertIV <- lm(children ~ frsthalf.IV + age + agesq, data=fertil)
Ferteduc <- lm(children ~ educ + age + agesq , data=fertil)

stargazer(EducIV, FertIV, Ferteduc, type = "latex", single.row = TRUE, no.space = TRUE, column.sep.width = "3pt")
...
```{r echo=FALSE, results='asis'}
reg1<- lm(idlnchld ~ tv , data = fertil)
reg2<- lm(idlnchld ~ tv + educ , data = fertil)
reg3<- lm(idlnchld ~ tv + educ + age + agesq, data = fertil)
reg4<- lm(idlnchld ~ tv + educ + age + agesq + evermarr , data = fertil)
stargazer(reg1, reg2, reg3, reg4, type="latex", single.row = TRUE, no.space = TRUE, column.sep.width = "3pt")
```
